...
export function ActivityRings({ size = 200, stroke = 18, gap = 4, rings }: ActivityRingsProps) {
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="block">
      <defs>
        {rings.map((r, i) => (
          <linearGradient key={`grad-${i}`} id={`ring-grad-${i}`} x1="0" y1="0" x2="1" y2="1">
        ...
        return (
          <g key={i}>
            ...
            <motion.circle
              ...
              transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: 0.12 + i * 0.08 }}
            />
          </g>
        );
      })}
